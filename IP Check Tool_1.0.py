...
def ip_status():
    ...
    for ip in endpointList:
        if stop == True:
            root.update()
            response = subprocess.Popen(["ping", "-n", "1", "-w", "200", ip],
                                        shell=True).wait()  # ["ping", "-n", "1", "-w", "200", ip]
            if response == 0:
                ipdict = {ip: "UP"}
...
namelabel = tk.Label(frame, borderwidth=3, relief="groove", text="Check IP/DNS hostname status ", fg="#008000",
                     font=("Italic", 12))
